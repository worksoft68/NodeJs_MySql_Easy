...
const pageTitleIndex = 'Ban Management';
...
router.get('(/searchget)?', async (req, res, next) => {
	let userLogin = publicFunction.getUserLogin(req, res);
	QuyenHeThongModel.kiemTraQuyenTuyCap(userLogin.UsersId, collection, res)
	let params = await ParamsHelpers.createParam(req);
	let languageString = await publicFunction.getLanguage(collection + ".ini");
	let item = {
		TenBan: await ParamsHelpers.getParam(req.query, 'TenBan', ''),
		GhiChu: await ParamsHelpers.getParam(req.query, 'GhiChu', ''),
	};
	item = clean(item);
...
router.delete('/deletebyid', middlesetHeader, async (req, res, next) => {
	...
	let item = Object.assign(req.body);
	item = clean(item);
	await MainModel.deleteById(item.IdBanKhoaChinh, userLogin).then((result) => {
		res.send({
			'success': result
		});
	});
