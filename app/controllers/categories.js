...
module.exports = {
	...
	indexString: async function(req, res, next) {
		...
		const catalogue = [];
		let categorieList = await MainModel.listItemForDropdown(req.user);
...
// end searchGet =================================================================================================
	...
	save: async function(req, res, next) {
		...
		uploadFile(req, res, async (errUpload) => {
			...
			let item = Object.assign(req.body);
			item = clean(item);
			if ((item.is_show_homepage == 'on') || (item.is_show_homepage == 'yes') || (item.is_show_homepage == 'true') || (item.is_show_homepage == '1'))
				item.is_show_homepage = 1;
			else item.is_show_homepage = 0;
