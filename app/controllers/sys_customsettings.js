...
// end searchGet =================================================================================================
	...
	save: async function(req, res, next) {
		...
		uploadFile(req, res, async (errUpload) => {
			...
			let item = Object.assign(req.body);
			item = clean(item);
			if ((item.is_use_ckeditor == 'on') || (item.is_use_ckeditor == 'yes') || (item.is_use_ckeditor == 'true') || (item.is_use_ckeditor == '1'))
				item.is_use_ckeditor = 1;
			else item.is_use_ckeditor = 0;

			if ((item.status == 'on') || (item.status == 'yes') || (item.status == 'true') || (item.status == '1'))
				item.status = 1;
			else item.status = 0;

			if ((item.is_system == 'on') || (item.is_system == 'yes') || (item.is_system == 'true') || (item.is_system == '1'))
				item.is_system = 1;
			else item.is_system = 0;

			//console.log(item);
			let taskCurrent = (typeof item !== "undefined" && item.idHidden != "") ? "edit" : "add";
			...
			if (errors != false) {
				...
				//	console.log(item["value_setting"].length);
