...
async function listItems(params, userLogin, options = null){
	...
	let sql = "select top " + params.pagination.totalItemsPerPage + " * from(" +
		" SELECT A.id, A.role_id, C.aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa, A.functions, B.description, A.fullauthority, A.addnew, A.updates, A.readonly, A.full_of_yourself, A.permission1, A.permission2, A.permission3, A.permission4, A.permission5, A.permission6, A.permission7, A.permission8, A.permission9, A.permission10, A.user_id_created, A.user_name_created, A.datetime_created, A.user_id_modified, A.user_name_modified "+
		", ROW_NUMBER() over(order by id desc) as RowNumber, COUNT(*) OVER() as TotalRecord " +
		" FROM sys_role_permissions A inner join sys_roles C on A.role_id = C. inner join sys_function_for_permissions B on A.functions = B.function_name  where (id > 0 )"; 
		if(options.functions != ""){
			dataParams.functions = "%" + options.functions + "%";
...
async function getById( id, userLogin = null){
	let sql = ` SELECT A.id, A.role_id, C.aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa, A.functions, B.description, A.fullauthority, A.addnew, A.updates, A.readonly, A.full_of_yourself, A.permission1, A.permission2, A.permission3, A.permission4, A.permission5, A.permission6, A.permission7, A.permission8, A.permission9, A.permission10, A.user_id_created, A.user_name_created, A.datetime_created, A.user_id_modified, A.user_name_modified
				FROM sys_role_permissions A
				inner join sys_roles C on A.role_id = C. inner join sys_function_for_permissions B on A.functions = B.function_name 
				where A.id = ${id} `;
